def dumberguesser():
    ...
    while True:
        response = input(f'Is it {guess} [Y/n]?').lower()[0] or 'y' # Defaults to y (yes, you have to type 'n' each time it's wrong)
        correct = responses.get(response, True) # If not y or n, default to True (y)
        if correct:
